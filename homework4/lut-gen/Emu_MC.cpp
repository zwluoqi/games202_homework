...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
    ...
    for (int i = 0; i < sample_count; i++) {
      // TODO: To calculate (fr * ni) / p_o here
        const auto& dir = sampleList.directions[i];
        const auto& pdf = sampleList.PDFs[i];

        auto NdotL = std::max(0.0f,dot(N, dir));
        auto H = normalize(dir + V);

        float denominator = std::max(0.00001f,4.0f * NdotV * NdotL);
        auto fr = GeometrySmith(roughness, NdotV, NdotL) * DistributionGGX(N, H, roughness) / denominator;


        auto GXX = fr * NdotL / pdf;
        A += GXX;
        B += GXX;
        C += GXX;
    }
...
int main() {
    ...
    for (int i = 0; i < resolution; i++) {//roughness iter
        for (int j = 0; j < resolution; j++) {//NV iter
