...
class MeshRender {
	...
	bindGeometryInfo() {
		...
		if (this.mesh.hasVertices) {
			...
			gl.vertexAttribPointer(
				...
				stride,
				offset);
				var gg = this.shader.program.attribs[this.mesh.verticesName];
	...
	draw(camera) {
		...
		for (var ii = 0; ii < 3; ++ii) {
			var aPrecomputeLTII = this.shader.program.attribs['aPrecomputeLT'] + ii;
			gl.enableVertexAttribArray(aPrecomputeLTII);
			gl.vertexAttribPointer(aPrecomputeLTII, 3, gl.FLOAT, false, 36, ii * 12);
		}

		// Bind attribute mat3 - LT
		
		let mat3ValueArray = [];
		if(precomputeL_RGBMat3.length >0){
			mat3ValueArray = getMat3ValueFromRGB(precomputeL_RGBMat3);
		}else{
			mat3ValueArray = getMat3ValueFromRGB(precomputeL[guiParams.envmapId]);
		...
		gl.uniformMatrix3fv(
			this.shader.program.uniforms.uLightMat3ColorR,
			false,
			mat3ValueArray[0]);
		gl.uniformMatrix3fv(
			this.shader.program.uniforms.uLightMat3ColorG,
			false,
			mat3ValueArray[1]);
		gl.uniformMatrix3fv(
			this.shader.program.uniforms.uLightMat3ColorB,
			false,
			mat3ValueArray[2]);
